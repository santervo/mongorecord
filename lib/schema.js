...
Schema.prototype.sanitizeProperty = function(attrs, value, key) {
  ...
  if(!this.isMassAssignableProperty(key)) {
    return attrs;
  }
  if(property.type === Object) {
    attrs[key] = property.subSchema.sanitizeData(value);
  }
  else if(property.type === Array) {
    attrs[key] = _.map(value, function(item) {
      return property.subSchema.sanitizeData({item: item}).item;
    });
  }
  else {
    attrs[key] = cast(property.type, value);
  }
  return attrs;
};

Schema.prototype.isMassAssignableProperty = function(key) {
  return this.properties[key] && !this.properties[key].protected;
};
...
function normalizeProperties(properties) {
  properties = _.reduce(properties, function(properties, def, key) {
    var property = properties[key] = normalizeProperty(def);
    if(property.type === Object) {
      property.subSchema = new Schema(property.properties);
    }
    else if(property.type === Array) {
      var subDef = { 
        item: { 
      ...
      property.subSchema = new Schema(subDef);
    }
    return properties;
  }, {});
  return properties;
